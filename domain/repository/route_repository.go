...
func (u *RouteRepository) InitTable() error {
	return u.mysqlDb.CreateTable(&model.Route{}, &model.Route{}).Error
}
...
func (u *RouteRepository) DeleteRouteByID(routeID int64) error {
	...
	//开始删除
	if err := u.mysqlDb.Where("id=?", routeID).Delete(&model.Route{}).
		Error; err != nil {
		tx.Rollback()
		common.Error(err)
		return err
	...
	if err := u.mysqlDb.Where("route_id = ?", routeID).Delete(&model.RoutePath{}).
		Error; err != nil {
		tx.Rollback()
		common.Error(err)
		return err
